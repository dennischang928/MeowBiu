...
void LVGL_Setup()
{
    ...
    size_t buffer_lines = 150; // Larger buffer

    // Create TWO buffers for double buffering
    lvbuf = heap_caps_malloc(EXAMPLE_LCD_H_RES * buffer_lines * sizeof(lv_color_t), MALLOC_CAP_DMA);
    assert(lvbuf);
    // Add second buffer
    lv_color_t *lvbuf2 = heap_caps_malloc(EXAMPLE_LCD_H_RES * buffer_lines * sizeof(lv_color_t), MALLOC_CAP_DMA);
    assert(lvbuf2);
    ...
    lv_display_set_buffers(disp,
                           lvbuf, lvbuf2, // Pass both buffers
                           EXAMPLE_LCD_H_RES * buffer_lines * sizeof(lv_color_t),
