...
try:
    ...
    HAS_ANTIALIAS_SUPPORT = True
except ImportError:
    HAS_ANTIALIAS_SUPPORT = False
    logging.warning("NumPy and/or PIL not found. A1 anti-aliasing will be disabled. Install with: pip install numpy pillow")
...
def apply_a1_antialias(filename: str, threshold: int = 128) -> str:
    """
    Apply anti-aliasing preprocessing for A1 (1-bit alpha) conversion.
    Uses Floyd-Steinberg dithering to create smooth edges.
    
    Returns: path to the processed image (overwrites original)
    """
    if not HAS_ANTIALIAS_SUPPORT:
        logging.warning("Skipping A1 anti-aliasing (numpy/PIL not available)")
        return filename
    ...
    try:
        ...
        h_grad = np.abs(padded[2:, 1:-1] - padded[:-2, 1:-1])
        v_grad = np.abs(padded[1:-1, 2:] - padded[1:-1, :-2])
        edge_strength = np.sqrt(h_grad**2 + v_grad**2)
        ...
        # Floyd-Steinberg dithering with edge-aware threshold
        dithered_alpha = alpha.copy()
        height, width = dithered_alpha.shape
        
        for y in range(height - 1):
            for x in range(1, width - 1):
                old_val = dithered_alpha[y, x]
                
                # Edge-aware threshold adjustment
                edge_factor = edge_strength[y, x]
                adaptive_threshold = threshold + (edge_factor - 0.5) * 40
                
                new_val = 255 if old_val >= adaptive_threshold else 0
                dithered_alpha[y, x] = new_val
                error = old_val - new_val
                
                # Distribute error
                if edge_factor > 0.3:
                    dithered_alpha[y, x + 1] += error * 7 / 16
                    dithered_alpha[y + 1, x - 1] += error * 3 / 16
                    dithered_alpha[y + 1, x] += error * 5 / 16
                    dithered_alpha[y + 1, x + 1] += error * 1 / 16
                else:
                    dithered_alpha[y, x + 1] += error * 5 / 16
                    dithered_alpha[y + 1, x - 1] += error * 2 / 16
                    dithered_alpha[y + 1, x] += error * 3 / 16
                    dithered_alpha[y + 1, x + 1] += error * 1 / 16
        
        # Clamp and apply slight blur
        dithered_alpha = np.clip(dithered_alpha, 0, 255).astype(np.uint8)
        alpha_pil = Image.fromarray(dithered_alpha, mode='L')
        dithered_alpha_smooth = np.array(alpha_pil.filter(ImageFilter.GaussianBlur(radius=0.4)))
        
        # Reconstruct image
        result = img_array.copy()
        result[:, :, 3] = dithered_alpha_smooth
        result_img = Image.fromarray(result, mode='RGBA')
        
        # Overwrite original
        result_img.save(filename)
        logging.info(f"Applied A1 anti-aliasing to {filename}")
        
    except Exception as e:
        logging.warning(f"Failed to apply A1 anti-aliasing: {e}")
...
class LVGLImage:
    ...
    def _png_to_alpha_only(self, cf: ColorFormat, filename: str):
        # Apply anti-aliasing preprocessing for A1 format
        if cf == ColorFormat.A1 and HAS_ANTIALIAS_SUPPORT:
            logging.info(f"Applying A1 anti-aliasing to {filename}")
            filename = apply_a1_antialias(filename, threshold=128)
...
def main():
    ...
    parser.add_argument('--a1-antialias', action='store_true',
                    help="apply anti-aliasing preprocessing for A1 (1-bit alpha) format to reduce jagged edges", default=True)
